...
def show_home():
    ...
    with col1:
        st.image(img1_path, width=250, caption="서울시 강남구 / 전세 2억")
    with col2:
        st.image(img2_path, width=250, caption="경기도 성남시 / 월세 80만 원")
...
def show_filters():
    ...
    housing_type = st.radio("선호 주거 형태를 선택하세요", ["원룸", "투룸/오피스텔/빌라"])
...
def show_preference_chart():
    ...
    fig = px.area(df, x="Date", y=["원룸", "투룸/오피스텔/빌라"], title="선택한 매물 선호도")
    fig.update_layout(yaxis_title="선호도", xaxis_title="날짜")
    st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    menu = st.sidebar.radio("선택", ["홈", "조건 선택", "추천 결과", "이용자 선호도", "문의하기"])
    if menu == "홈":
        show_home()
    elif menu == "조건 선택":
        show_filters()
